...
// cpuid_low is implemented in cpu_x86.s for gc compiler
// and in cpu_gccgo.c for gccgo.
func cpuid_low(eaxArg, ecxArg uint32) (eax, ebx, ecx, edx uint32)

// xgetbv_low with ecx = 0 is implemented in cpu_x86.s for gc compiler
// and in cpu_gccgo.c for gccgo.
func xgetbv_low(index uint32) (eax, edx uint32)

// VendorIdentificator like "GenuineIntel" or "AuthenticAMD"
var VendorIdentificator string

// ProcessorBrand like "Intel(R) Core(TM) i7-4770HQ CPU @ 2.20GHz"
...
// MaxLogocalCPUID maximum number of addressable IDs for logical processors in this physical package
var MaxLogicalCPUID uint32

// InitialAPICID initial APIC ID
...
// CacheDescriptor represents a CPU cache descriptors.
...
// CacheDescriptors cache descriptor's array.
//
// See CacheDescriptor type for more information.
//
// 	for _, cacheDescription := range cpu.CacheDescriptors {
...
func HasExtendedFeature(feature ExtendedFeature) bool {
...
func HasExtraFeature(feature ExtraFeatureFlag) bool {
...
func HasThermalAndPowerFeature(feature ThermalAndPowerFeature) bool {
...
// leaf0 detect maxInputValue, VendorIdentificator and brandID.
func leaf0() {
	eax, ebx, ecx, edx := cpuid(0, 0)

	maxInputValue = eax

	VendorIdentificator = string(uint32sToBytes(ebx, edx, ecx))
	VendorID = Vendors[VendorIdentificator]
}

// leaf1 detect some cpuid instructions.
func leaf1() {
	if maxInputValue < 1 {
		return
	}
	...
	FeatureFlags = FeatureFlag((uint64(edx) << 32) | uint64(ecx))
}

// leaf2 detect CacheDescriptors size which Intel specific.
func leaf2() {
	if VendorID != INTEL || maxInputValue < 2 {
		return
	}

	bytes := uint32sToBytes(cpuid_low(2, 0))
...
// leaf3 detect SerialNumber which Intel specific.
func leaf3() {
	if VendorID != INTEL || maxInputValue < 3 {
		return
	}
...
// leaf4 detect CacheDescriptors size which Intel specific.
func leaf4() {
	if VendorID != INTEL || maxInputValue < 4 {
		return
	}
...
// leaf5 detect Monitor related cpu instructions.
func leaf5() {
	if maxInputValue < 5 {
		return
	}
...
// leaf6 detect thermal and power features.
func leaf6() {
	...
	ThermalAndPowerFeatureFlags = ThermalAndPowerFeature((eax & 0xFFFF) | (ecx << 16))
	ThermalSensorInterruptThresholds = ebx & 7
}

// leaf7 detect extended feature flags.
func leaf7() {
	_, ebx, ecx, _ := cpuid_low(7, 0)
	ExtendedFeatureFlags = ExtendedFeature((uint64(ecx) << 32) | uint64(ebx))
}

// leaf0x80000000 detect max extended input value.
func leaf0x80000000() {
	maxExtendedInputValue, _, _, _ = cpuid_low(0x80000000, 0)
}

// leaf0x80000001 detect extra feature flags.
func leaf0x80000001() {
	...
	ExtraFeatureFlags = ExtraFeatureFlag((uint64(edx) << 32) | uint64(ecx))
}
...
func leaf0x80000004() {
	...
	ProcessorBrand += string(uint32sToBytes(cpuid_low(0x80000002, 0)))
	ProcessorBrand += string(uint32sToBytes(cpuid_low(0x80000003, 0)))
	ProcessorBrand += string(uint32sToBytes(cpuid_low(0x80000004, 0)))
}
...
// leaf0x80000006 looks at the Processor Brand String in leaves 0x80000006.
func leaf0x80000006() {
...
// leaf02Descriptors represents a map of CacheDescriptor.
//
...
var leaf02Descriptors = map[int16]CacheDescriptor{
	...
	0x40: {-1, DATA_CACHE, "No 2nd-level cache or, if processor contains a valid 2nd-level cache, no 3rd-level cache", -1, -1, -1, -1, 0},
	...
	0xB1:              {-1, TLB, "Instruction TLB 2M pages 4 way 8 entries or 4M pages 4-way, 4 entries", 2 * 1024, 4, -1, 8, 0},
	...
	0xC3:              {-1, STLB, "Shared 2nd-Level TLB: 4 KByte /2 MByte pages, 6-way associative, 1536 entries. Also 1GBbyte pages, 4-way,16 entries.", 4, 6, -1, 1536, 0},
	...
	0xFF:              {-1, NULL, "CPUID leaf 2 does not report cache descriptor information, use CPUID leaf 4 to query cache parameters", -1, -1, -1, -1, 0},
}

// uint32sToBytes returns the uint32 args to bytes slice.
func uint32sToBytes(uint32s ...uint32) (result []byte) {
	result = make([]byte, len(uint32s)*4)

	for i, u32 := range uint32s {
		result[i] = byte((u32) & 0xFF)
		result[i+1] = byte((u32 >> 8) & 0xFF)
		result[i+2] = byte((u32 >> 16) & 0xFF)
		result[i+3] = byte((u32 >> 24) & 0xFF)
	}
...
// Vendor represents a CPU vendor.
type Vendor int

// VendorID id of CPU vendor.
var VendorID Vendor
...
const (
	...
	CYRIX
	HYGON
	HYPERV // Microsoft Hyper-V or Windows Virtual PC
	INTEL
...
// ExtraFeatureFlag represents a feature type.
type FeatureFlag uint64

// FeatureFlags flag of feature.
var FeatureFlags FeatureFlag

// list of features.
const (
	SSE3 = FeatureFlag(1) << iota
	PCLMULQDQ
	DTES64
	MONITOR
	DSI_CPL
...
// FeatureNames map of CPU feature names.
...
// ExtraFeatureFlag represents a extra feature type.
type ExtraFeatureFlag uint64

// ExtraFeatureFlags flag of extra feature.
var ExtraFeatureFlags ExtraFeatureFlag

// list of ExtraFeatureNames.
const (
	LAHF_LM = ExtraFeatureFlag(1) << iota
	CMP_LEGACY
	SVM
	EXTAPIC
	CR8_LEGACY
...
// ExtraFeatureNames map of extra feature names.
var ExtraFeatureNames = map[ExtraFeatureFlag]string{ // From leaf 8000, 0001
	LAHF_LM:      "LAHF_LM",
...
// ExtendedFeature represents an extended feature type.
type ExtendedFeature uint64

// ExtendedFeatureFlags flags of extended features.
var ExtendedFeatureFlags ExtendedFeature

// ExtendedFeatureNames map of extended feature names.
var ExtendedFeatureNames = map[ExtendedFeature]string{ // from leaf07
	FSGSBASE:              "FSGSBASE",
...
// list of ExtendedFeature.
const (
	FSGSBASE = ExtendedFeature(1) << iota
	IA32_TSC_ADJUST
	_
	BMI1
	HLE
...
// ThermalAndPowerFeature represents a thermal and power feature type.
type ThermalAndPowerFeature uint32

// ThermalAndPowerFeatureFlags flags of thermal and power features.
var ThermalAndPowerFeatureFlags ThermalAndPowerFeature

// ThermalAndPowerFeatureNames map of thermal and power feature names.
var ThermalAndPowerFeatureNames = map[ThermalAndPowerFeature]string{ // from leaf06
	ARAT:                      "ARAT",
	PLN:                       "PLN",
	ECMD:                      "ECMD",
	PTM:                       "PTM",
...
// lit of Thermal and Power Management features.
const (
	// EAX bits 0-15
	TEMPERATURE_SENSOR        = ThermalAndPowerFeature(1) << iota // Digital temperature sensor
